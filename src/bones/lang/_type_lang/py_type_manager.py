...
class PyTypeManager:
    ...
    def _intersectionTypeList(self, types):
        ...
        for t in types:
            if t.bmtid == bmtint:
                interTypes = t.btype.types if isinstance(t, TBC) else t.types
                typelist.extend(interTypes)
                group = [self.rootSpace(tChild) for tChild in interTypes]
                group.append(self.rootSpace(t))
                allRoots.extend(set(group))
            else:
                typelist.append(t)
                allRoots.append(self.rootSpace(t))
        allRoots = [t for t in allRoots if t]
        if len(set(allRoots)) != len(allRoots):
            raise BTypeError('The types share common orthogonal spaces')
        return tuple(sorted(set(typelist), key=lambda t: t.id))
    ...
    def struct(self, names, btypes, *, btype=Missing):
        if (struct := self._structByFields.get((names, btypes), Missing)) is Missing:
            struct = Struct(bmtstr, self._nextId(btype), names, btypes)
            struct = self._structByFields[(names, btypes)] = self._add(struct, tbc=btype)
        return struct
