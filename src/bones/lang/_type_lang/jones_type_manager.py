...
class JonesTypeManager:
    ...
    def initAtom(self, *, explicit=Missing, space=Missing, implicitly=Missing, name='', btype=Missing):
        if (btype := self._tm.lookup(name)).id and self._tm.bmetatypeid(btype) != bmtnul:
            btype = self._tm.checkAtom(btype, explicit=explicit or False, space=space or None, implicitly=implicitly or None)
        else:
            btype = btype if btype.id else None
            btype = self._tm.initAtom(btype=btype, explicit=explicit or False, space=space or None, implicitly=implicitly or None)
        return btype
    ...
    def reserve(self, space, btype=Missing):
        return self._tm.reserve(space=space or None)
