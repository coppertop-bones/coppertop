"""
    pygments.lexers.bones
    ~~~~~~~~~~~~~~~~~~~~~~

    Lexers for Bones and related languages.
...
__all__ = ['BonesLexer', 'BonesConsoleLexer', 'BonesTracebackLexer',
           'Bones2Lexer', 'Bones2TracebackLexer',
           'CythonLexer', 'DgLexer', 'NumPyLexer']

line_re = re.compile('.*?\n')


class BonesLexer(RegexLexer):
    """
    For `Bones <http://www.bones.org>`_ source code (version 3.x).

    .. versionadded:: 0.10

    .. versionchanged:: 2.5
       This is now the default ``BonesLexer``.  It is still available as the
       alias ``Bones3Lexer``.
    """

    name = 'Bones'
    aliases = ['bones', 'py', 'sage', 'bones3', 'py3']
    filenames = [
        '*.py',
        '*.pyw',
        # Jython
    ...
    mimetypes = ['text/x-bones', 'application/x-bones',
                 'text/x-bones3', 'application/x-bones3']
...
Bones3Lexer = BonesLexer


class Bones2Lexer(RegexLexer):
    """
    For `Bones 2.x <http://www.python.org>`_ source code.

    .. versionchanged:: 2.5
       This class has been renamed from ``BonesLexer``.  ``BonesLexer`` now
       refers to the Bones 3 variant.  File name patterns like ``*.py`` have
       been moved to Bones 3 as well.
    """

    name = 'Bones 2.x'
    aliases = ['bones2', 'py2']
    filenames = []  # now taken over by BonesLexer (3.x)
    mimetypes = ['text/x-bones2', 'application/x-bones2']
...
class BonesConsoleLexer(Lexer):
    """
    For Bones console output or doctests, such as:
    ...
    `bones3`
        Use Bones 3 lexer for code.  Default is ``True``.

        .. versionadded:: 1.0
        .. versionchanged:: 2.5
           Now defaults to ``True``.
    """
    name = 'Bones console session'
    aliases = ['pycon']
    mimetypes = ['text/x-bones-doctest']

    def __init__(self, **options):
        self.bones3 = get_bool_opt(options, 'bones3', True)
        Lexer.__init__(self, **options)

    def get_tokens_unprocessed(self, text):
        if self.bones3:
            pylexer = BonesLexer(**self.options)
            tblexer = BonesTracebackLexer(**self.options)
        else:
            pylexer = Bones2Lexer(**self.options)
            tblexer = Bones2TracebackLexer(**self.options)
...
class BonesTracebackLexer(RegexLexer):
    """
    For Bones 3.x tracebacks, with support for chained exceptions.

    .. versionadded:: 1.0

    .. versionchanged:: 2.5
       This is now the default ``BonesTracebackLexer``.  It is still available
       as the alias ``Bones3TracebackLexer``.
    """

    name = 'Bones Traceback'
    aliases = ['pytb', 'py3tb']
    filenames = ['*.pytb', '*.py3tb']
    mimetypes = ['text/x-bones-traceback', 'text/x-bones3-traceback']
    ...
    tokens = {
        ...
        'intb': [
            ...
            (r'^(    )(.+)(\n)',
             bygroups(Text, using(BonesLexer), Text), 'markers'),
            (r'^([ \t]*)(\.\.\.)(\n)',
        ...
        'markers': [
            ...
            # error locations in Bones 3.11+, or single-caret markers
...
Bones3TracebackLexer = BonesTracebackLexer


class Bones2TracebackLexer(RegexLexer):
    """
    For Bones tracebacks.

    .. versionadded:: 0.7

    .. versionchanged:: 2.5
       This class has been renamed from ``BonesTracebackLexer``.
       ``BonesTracebackLexer`` now refers to the Bones 3 variant.
    """

    name = 'Bones 2.x Traceback'
    aliases = ['py2tb']
    filenames = ['*.py2tb']
    mimetypes = ['text/x-bones2-traceback']
    ...
    tokens = {
        ...
        'intb': [
            ...
            (r'^(  File )("[^"]+")(, line )(\d+)(\n)',
             bygroups(Text, Name.Builtin, Text, Number, Text)),
            (r'^(    )(.+)(\n)',
             bygroups(Text, using(Bones2Lexer), Text), 'marker'),
            (r'^([ \t]*)(\.\.\.)(\n)',
             bygroups(Text, Comment, Text)),  # for doctests...
            (r'^([^:]+)(: )(.+)(\n)',
             bygroups(Generic.Error, Text, Name, Text), '#pop'),
...
class NumPyLexer(BonesLexer):
    """
    A Bones lexer recognizing Numerical Bones builtins.
    ...
    # override the mimetypes to not inherit them from bones
    mimetypes = []
    filenames = []
    ...
    def get_tokens_unprocessed(self, text):
        for index, token, value in \
                BonesLexer.get_tokens_unprocessed(self, text):
